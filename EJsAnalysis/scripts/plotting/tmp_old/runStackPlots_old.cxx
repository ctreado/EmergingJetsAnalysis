...
{ 
  ...
  //ptitle.push_back( "MC16d (old official)" );
  //ptitle.push_back( "MC16d (update test)"  );
  //ptitle.push_back( "data"  );
  //ptitle.push_back( "truth (pythia test)"      );
  //ptitle.push_back( "truth (mc15 test)"        );
  //ptitle.push_back( "truth (mc15 update test)" );
  ptitle.push_back( "truth (mc15 update)"      );
  ...
  for ( size_t i = 0; i != ptitle.size(); ++i ) {
    ...
    //std::cout << "on ith iteration: " << i << " -- " << ptitle.at(i) << std::endl;

    // set plot type
    TString lowtitle = ptitle.at(i); lowtitle.ToLower();
    TString ptype    = "";
    if      ( lowtitle.Contains("mc16d") && lowtitle.Contains("old official")     ) ptype = "mc16d_oldoff";
    else if ( lowtitle.Contains("mc16d") && lowtitle.Contains("update test")      ) ptype = "mc16d_updatetest";
    else if ( lowtitle.Contains("data")                                           ) ptype = "data";
    else if ( lowtitle.Contains("truth") && lowtitle.Contains("pythia test")      ) ptype = "truth_pytest";
    else if ( lowtitle.Contains("truth") && lowtitle.Contains("mc15 test")        ) ptype = "truth_mc15test";
    else if ( lowtitle.Contains("truth") && lowtitle.Contains("mc15 update test") ) ptype = "truth_mc15updatetest";
    else if ( lowtitle.Contains("truth") && lowtitle.Contains("mc15 update")      ) ptype = "truth_mc15update";
    ...
    for ( size_t j = 0; j != pregion.size(); ++j ) {
      ...
      if ( !ptype.Contains("data") ) {
    	...
    	makeStackPlots( "truthJet_truthVtxDispCount",   title, "truth-jet-matched displaced (r > 0 mm) truth vertex count",         "", ptype, region, "truthJet" );
    	...
    	makeStackPlots( "truthEJ_truthVtxDispCount",   title, "truth-emerging-jet-matched displaced (r > 0 mm) truth vertex count",         "", ptype, region, "truthJet" );
    	...
    	makeStackPlots( "truthQCDJ_truthVtxDispCount",   title, "truth-qcd-jet-matched displaced (r > 0 mm) truth vertex count",         "", ptype, region, "truthJet" );
    	...
    	makeStackPlots( "darkJet_truthVtxDispCount",   title, "dark-jet-matched displaced (r > 0 mm) truth vertex count",         "", ptype, region, "darkJet" );
    	...
    	makeStackPlots( "truthVtx_disp_n", title, "n displaced (r > 0 mm) truth vertices", "", ptype, region, "truthVtx" );
