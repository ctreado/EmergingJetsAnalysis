...
##******************************************
#variation on xAODAnaHelper's
#downloadAndMerge.py
#download and merge output datasets produced by EJs analysis code on the GRID
# --> capability to download n individual random files instead of whole dataset
...
parser.add_argument("--nrandom", dest='nrandom', default=-1,
     help="Number of random individual files to download from dataset. -1 for downloading the entire dataset.")
  ...
  for idataset, dataset in enumerate(datasetList):
    ...
    if args.doFax:
      ...
      if args.nrandom<=0:
        ...
        print "rucio download "+dataset+" --nrandom "+args.nrandom+" --ndownloader 5"
        os.system("rucio download "+dataset+" --nrandom "+args.nrandom+" --ndownloader 5")
